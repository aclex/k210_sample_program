...
int cambus_scan(uint16_t* manuf_id, uint16_t* device_id);

int ov2640_init(void)
{
    uint16_t v_manuf_id;
    uint16_t v_device_id;
	const uint8_t addr = cambus_scan(&v_manuf_id, &v_device_id);

	if (!addr)
	{
		printf("OV2640 device not found.\r\n");
		return -1;
	}
    ...
    for (index = 0; ov2640_config[index][0]; index++)
        dvp_sccb_send_data(addr, ov2640_config[index][0], ov2640_config[index][1]);
    return 0;
}
...
int cambus_scan(uint16_t* manuf_id, uint16_t* device_id)
{
	...
	{
		ov2640_read_id(addr, manuf_id, device_id);
